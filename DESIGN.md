...
Since many routes (such as `/diagonalize`, `/signature`, and `/svd`) need matrix input validation, it was good design to abstract a function that performed matrix dimension validation in helpers.py. This is good practice as opposed to copying and pasting the same kind of code. 
## Design Decisions
